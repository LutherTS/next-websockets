...
export default function WebSocketsClientPage({
  ...
  return (
    // @ts-ignore-xjsx
    <main class="flex min-h-screen items-center justify-center bg-gradient-to-b from-gray-50 to-gray-100">
      {/* @ts-ignore-xjsx */}
      <div class="mx-4 flex h-[90dvh] w-full max-w-2xl flex-col rounded-xl border border-gray-200 bg-white shadow-lg md:h-[80dvh]">
        <div
        ...
        {/* @ts-ignore-xjsx */}
        <div class="flex flex-1 flex-col gap-y-4 overflow-y-auto bg-gray-50 px-6 pt-6">
          {messages.map(({ value: message, id, username }) => (
            <div
              key={id}
              // @ts-ignore-xjsx
              class="rounded-lg border border-gray-100 bg-white p-4 shadow-sm transition-all hover:shadow-md"
            >
              {/* @ts-ignore-xjsx */}
              <p class="font-medium text-gray-800">
                ...
                {username && (
                  <>
                    {" "}
                    {/* @ts-ignore-xjsx */}
                    <span class="text-gray-500 italic">– {username}</span>
                  </>
                )}
              </p>
            </div>
          ...
          {/* @ts-ignore-xjsx */}
          <div class="pt-2" ref={bottomRef}></div>
        </div>

        <form
          onSubmit={broadcast}
          // @ts-ignore-xjsx
          class="rounded-b-xl border-t border-gray-100 bg-white p-6"
        >
          {/* @ts-ignore-xjsx */}
          <div class="flex flex-col gap-3 md:flex-row">
            <input
              ...
              type="text"
              // @ts-ignore-xjsx
              class="flex-1 rounded-lg border border-gray-200 px-4 py-3 transition-all focus:border-transparent focus:ring-2 focus:ring-blue-500 focus:outline-none"
